...
class SimpleAsyncCrawler(AbstractAsyncCrawler):
    ...
    async def _init_link_set(self) -> None:
        await self.add_link(self.base_url)

    async def crawl(self):
        while not self.done.is_set():
            if self.link_set:
                tasks = tuple(map(self.read_link, self.link_set))
                self.link_set.clear()
    ...
    async def read_link(self, url: str) -> None:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                await self.process_link(url, response.content)
    ...
    async def process_link(self, url: str, response: bytes) -> None:
        ...
        for link in bf4.find_all("a"):
            ...
            if href is not None:
                ...
                await self.add_link(url_)

        await self.read_content(url, connected_links, bf4)
    ...
    async def add_link(self, url: str) -> None:
